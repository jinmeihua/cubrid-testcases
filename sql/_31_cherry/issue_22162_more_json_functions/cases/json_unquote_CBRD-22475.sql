...
SELECT c->'$.name' FROM t WHERE i < 30;
SELECT JSON_UNQUOTE(c->'$.name') FROM t WHERE i < 30;
--CBRD-22475
SELECT c->>'$.name'  FROM t WHERE  i < 30;
...
SELECT c->'$[4]' FROM t order by 1;
SELECT * FROM t WHERE c->'$[0]' = 3 order by 1,2;
SELECT * FROM t WHERE c->'$[4][1]' IS NOT NULL order by 1,2;
SELECT c->'$[4][1]' FROM t order by 1;

SELECT i, JSON_UNQUOTE(json_extract(c,'$[2]')) FROM t where i >4;
SELECT i, JSON_UNQUOTE(c->'$[2]') FROM t where i >4;
--CBRD-22475
SELECT i, c->>'$[2]' FROM t where i >4;;
SELECT i, JSON_UNQUOTE(json_extract(c,'$[4][2]')) FROM t where i >4;
SELECT i, JSON_UNQUOTE(c->'$[4][2]') FROM t where i >4;;
SELECT i, c->>'$[4][2]' FROM t where i >4;;
